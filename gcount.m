function [Z, gnames] = gcount(G, d, subset, maxvalue, doplot, varargin)
%Z = gcount(G) counts the number of each element in group G.
...
%variables.
%
%Z = gcount(G, d) to work along dimension d (by default : first
%non-singleton dimension)
%
%Z = gcount(G, d, subset) to use a subset
%
%Z = gcount(G, d, subset, maxvalue)
%
%Z = gcount(G, d, subset, maxvalue, 'plot')
%Z = gcount(G, d, subset, maxvalue, 'plot', plotoptions) to use wu plot
%options (type help wu)
%
%See also GRPN, GSUM, GMEAN, GFUN, grphist, grp2idx
