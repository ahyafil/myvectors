%Z = grpfun(fun, Y, G)
%
%Z = grpfun(fun, Y, G, subset)
%
%[Z1 Z2 .. Zn ] = grpfun({fun1 fun2 ..funn}, Y, G)
%[Z1 Z2 .. Zn ] = grpfun({fun1 fun2 ..funn}, Y, G, 'nargout', N)
%
%Z = grpfun(fun, Y, G, 'max', M)
%(use 0 to ignore at that position)
%(use -1 to use max(X{i}) )
%
%grpfun(..., plotopt) to use plot options as in function wu (type 'help wu'
%for more information)
%grpfun(.... 'plotfunc', [i_raw i_error]) to use output i_raw for plotting
%raw values, and i_error for error values (default: 1st for raw and 2nd for
%error)
%grpfun(..., 'gnames', Gn) where Gn is a cell array of group names to add
%labels/values to non-grouping dimensions for plotting
%
%
%Z = grpfun(fun, Y, G, 'UniformOutput', bool)
%Z = grpfun(..., 'multidim', method) to use a specific method when there are multiple grouping dimensions :
...
%
%grpfun(...,'quantile', Q) to bin G data (see quantiles for more), Q being
%a vector or cell array of vectors (use empty to xxx)
%grpfun(..., 'histc', edges) replaces G data with bin number in which each
...
if nargin >=4,
    ...
    while v+3 <= nargin
        switch class(varargin{v}),
                ...
                switch lower(varargin{v})
                   ...
                   case 'histc',
                    ...
                    case 'plotfun',
                        ...
                        v = v + 1;
                        plotfun = varargin{v};
                    case {'color', 'curve','linewidth', 'linestyle', 'marker', 'markersize','barwidth', 'errorstyle', 'title', 'name', 'xtick', 'permute','axis'},
                        doplot = 1;
            ...
            otherwise
                error('grpfun:unsupportedparameter', 'unsupported parameter type for parameter #%d',v+3);
        end
...
if ~isvector(Y),
    ...
    for d = length(dim2)
        ...
        extrad = thisd(2:end);
        nextrad = length(extrad);
        permorder(end+(1:nextrad)) = permorder(extrad);
        permorder(extrad) = ndim + (1:nextrad);
        ndim = ndim + nextrad;
    end
...
if doplot,
    ...
    switch length(plotfun)  %plot only raw or raw and error values
        ...
        otherwise
            error('grpfun: can plot only if no more than two output matrices');
    end
